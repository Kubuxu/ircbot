...
hook.new("join", function(message, user, channel) if not hook.auth(user.nick) then return "Nope" else hook.irc:join(message) end end)
hook.new("leave", function(message, user, channel) if not hook.auth(user.nick) then return "Nope" else hook.irc:leave(message) end end)